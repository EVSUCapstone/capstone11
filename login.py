...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mssql+pyodbc://NIA\\SQLEXPRESS/evsuDB?trusted_connection=yes&driver=ODBC+Driver+17+for+SQL+Server'
...
def dashboard2():
    try:
        ...
        start_year = 2020
        end_year = max(yearly_programs.keys(), default=start_year)
        all_years = list(range(start_year, end_year + 1))  # Ensure the range starts from 2020

        program_counts = []
        for year in all_years:
            program_counts.append(yearly_programs.get(year, 0))  # If the year has no data, default to 0
...
def main_campus():
    try:
        ...
        programs = [
            {"projectid": row[0], "title": row[1], "leader": row[2]} for row in cursor.fetchall()
        ]
...
def add_program():
    try:
        ...
        sdg_goals = request.form.getlist('sdg[]')  # Adjusting to handle multiple SDG values
        if not sdg_goals:
            print("SDG values are missing or empty.")
            return jsonify({"status": "error", "message": "No SDG values provided."}), 400
        else:
        ...
        new_project_id = cursor.execute("SELECT ISNULL(MAX(projectid), 0) + 1 FROM dbo.Projects").fetchval()
        ...
        cursor.execute(f"INSERT INTO dbo.Projects ({columns}) VALUES ({placeholders})", list(data.values()))
        conn.commit()
    ...
    try:
        ...
        if project:
            ...
            for key, value in project_dict.items():
                if isinstance(value, datetime):
                    project_dict[key] = value.strftime('%Y-%m-%d')  # Format datetime to 'YYYY-MM-DD'
                elif value is None:
                    project_dict[key] = ''
                elif key == 'sdg' and value:
                    # Ensure SDG is returned as a clean comma-separated string
                    project_dict[key] = ','.join([sdg.strip() for sdg in value.split(',') if sdg.strip()])
...
def edit_program(projectid):
    try:
        ...
        # Get SDG values from the form, which are sent as a comma-separated string
        sdg_goals = request.form.getlist('sdg')
        sdg_values = ','.join(sdg_goals) if sdg_goals else None  # Store as comma-separated string

        print(f"SDG value during update: {sdg_values}")
        ...
        # Create update clauses and values
        update_clauses = ', '.join([f"{key}=?" for key in data.keys()])
        update_values = list(data.values()) + [projectid]

        # Execute the update query
        update_query = f"UPDATE dbo.Projects SET {update_clauses} WHERE projectid=?"
        cursor.execute(update_query, update_values)
        
        # Check if any rows were actually updated
        if cursor.rowcount == 0:
            conn.close()
            return jsonify({"status": "error", "message": "No program found with the given ID"}), 404
